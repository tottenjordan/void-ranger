...
![Cosmic Web scale: a compute node deployed ~300 Mpc out among the 2MRS galaxies forming the filamentary cosmic web, with the red comm line to Earth and the six-card metrics row showing a positive net gain](docs/images/far-future.png)

<sub><i>The full dashboard at the **Cosmic Web** scale: a compute node ~300 Mpc out in a void beyond the local galaxy field (2MRS), the red round-trip communication line back to Earth, the six-card metrics row (years over days), and the "In plain terms" summary below the map. Annotated walkthrough below.</i></sub>

This capture is the **Cosmic Web** scale (top-bar toggle) with a **100-billion-year** workload, the node placed in a void **~300 Mpc** out — well beyond the dense local galaxy field, where the dashed **red comm line** and the **300.0 Mpc** distance label lead out to it. (At megaparsec distances offloading only pays off for an astronomically large job — that's the lesson.) Reading the screen:

- The bright **galaxy field** is the 2MASS Redshift Survey tracing the **filamentary cosmic web**; the deployed node sits out in the sparse outskirts, joined to Earth (at the origin) by the dashed **red communication line** and a parallel **violet distance line**. The brightest **named galaxies** (Andromeda, Centaurus A, …) are labeled, and a **Map Key** in the controls row labels every element.
- The **metrics row** shows the result: *Distance* 300.00 Mpc, a *Server Clock Advantage* of **1.145× Earth** (the void's weak galaxy field runs the clock faster), an *Earth Compute Time* of ~87.4 billion years, a *Communication Cost* of ~1.96 billion years, an *Earth Wait Time* of ~89.3 billion years, and a **positive Net Gain of ~10.7 billion years** (green ▲) — offloading wins. (Each card shows years as the main value with the days equivalent beneath; the model computes in seconds internally.)
- The **In plain terms** panel translates that into words: *running this job on Earth would take 100 billion years; offloaded the same computation takes ~87.4 billion years of Earth compute plus ~1.96 billion years of signal round-trip — a ~89.3-billion-year wait, a net saving of ~10.7 billion years.*
- **Why such a huge job?** At megaparsec distances the round-trip light delay runs to billions of years, so the breakeven workload is enormous — only a job measured in *tens of billions of years* clears it. Smaller jobs read as a net loss; *Best spot for this task* finds the placement that maximizes net gain for whatever size you set. (Drop to the Solar Neighborhood scale and the same trade-off plays out with parsecs and thousands of years.)
...
> **Deep Field (grid-backed potential):** at the Deep Field scale the potential $\Phi$ is not summed over 22.5 M GLADE+ galaxies at query time — it is read from a **precomputed voxel grid** (raw J/kg, trilinearly interpolated), making the deepest-void search O(voxels) regardless of catalog size. The grid stores the *raw* potential; the teaching exaggeration is **auto-derived per grid** (applied **once** in `gravitational_dilation`, never baked into the grid) so the deepest void's clock advantage ≈ **1.06** on **any** grid — committed sample or full catalog — inside the documented **1.05–1.10** teaching band, with no saturation. Like the solar/cosmic exaggerations, it is a labeled teaching device — relative contrast is faithful, absolute magnitude is not. See [The Deep Field scale](docs/deep-field.md).
